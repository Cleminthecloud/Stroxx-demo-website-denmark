...
2. **Draft an article**: paste an idea and get a complete draft: article, excerpt, SEO title + description, a share-image suggestion, 3 to 6 related products from the real range (paste the item numbers into the Related products field) and a ready LinkedIn post. It knows the full product range by name and item number, and if your brief names a STROXX product outside the site's range it verifies it on the dealer sites with live web search instead of guessing, and tells you what it could not verify. Paste the pieces into a new News article document, add the image, publish.

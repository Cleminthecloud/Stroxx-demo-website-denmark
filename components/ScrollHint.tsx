/** Minimal animated scroll indicator — a mouse with a travelling wheel dot
 *  and a soft chevron pulse. Pure SVG/SMIL, no JS. */
export default function ScrollHint() {
  return (
    <div className="flex flex-col items-center gap-2 opacity-70">
      <svg width="24" height="38" viewBox="0 0 24 38" fill="none" aria-hidden>
        <rect x="1.5" y="1.5" width="21" height="35" rx="10.5" stroke="#8A9199" strokeWidth="1.4" />
        <circle cx="12" cy="9" r="2.4" fill="#fff">
          <animate attributeName="cy" values="8;18;8" dur="1.8s" repeatCount="indefinite" calcMode="spline" keySplines="0.4 0 0.2 1;0.4 0 0.2 1" />
          <animate attributeName="opacity" values="1;0.15;1" dur="1.8s" repeatCount="indefinite" />
        </circle>
      </svg>
      <svg width="16" height="9" viewBox="0 0 16 9" fill="none" aria-hidden>
        <path d="M1 1l7 6 7-6" stroke="#0088C2" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
          <animate attributeName="opacity" values="0.3;1;0.3" dur="1.8s" repeatCount="indefinite" />
        </path>
      </svg>
    </div>
  );
}

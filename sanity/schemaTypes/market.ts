...
export const market = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      title: 'Languages',
      type: 'array',
      description: 'Which languages this market publishes in. Belgium has two (nl + fr).',
      of: [defineArrayMember({ type: 'string' })],
      options: { list: LANGS },
      validation: (r) => r.required().min(1),
    }),
    defineField({ name: 'defaultLanguage', title: 'Default language', type: 'string', description: 'The language shown first for this market.', options: { list: LANGS } }),
    ...
    defineField({ name: 'active', title: 'Live', type: 'boolean', initialValue: false, description: 'Off = built but not yet published for visitors.' }),
    defineField({ name: 'dealerName', title: 'Dealer / distributor', type: 'string', description: 'e.g. Carl Ras, Meesenburg, Foussier, Lecot.' }),
